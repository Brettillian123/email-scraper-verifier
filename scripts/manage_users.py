...
def cmd_create(args):
    ...
    logger.info(f"Created user: {user.email} (ID: {user.id}, tenant: {user.tenant_id}, status: {status})")
...
def cmd_list(args):
    ...
    try:
        ...
        for row in rows:
            ...
            email = row["email"][:28] + ".." if len(row["email"] or "") > 30 else (row["email"] or "")
            ...
            if not row.get("is_approved"):
                print(f"{user_id:<20} {email:<30} \033[93m{approved:<10}\033[0m {active:<8} {superuser:<8} {created}")
            else:
                print(f"{user_id:<20} {email:<30} {approved:<10} {active:<8} {superuser:<8} {created}")
...
def main():
    ...
    create_parser.add_argument("--superuser", action="store_true", help="Grant superuser privileges (auto-approves)")
    ...
    limits_parser.add_argument("--max-verifications-per-day", type=int, help="Max verifications per day")
    limits_parser.add_argument("--max-verifications-per-month", type=int, help="Max verifications per month")
