...
# Company
cur.execute(
    "INSERT OR IGNORE INTO companies (name, domain, website_url) VALUES (?,?,?)",
    ("Acme Widgets", "acme.test", "https://acme.test"),
)

company_id = (
    cur.lastrowid
    or cur.execute("SELECT id FROM companies WHERE domain=?", ("acme.test",)).fetchone()[0]
)
...
# Email
cur.execute(
    """INSERT INTO emails (person_id, company_id, email, is_published, source_url, icp_score)
              VALUES (?,?,?,?,?,?)""",
...
cur.execute(
    """INSERT INTO verification_results (email_id, mx_host, status, reason, checked_at)
              VALUES (?,?,?,?,?)""",
    (email_id, "mx.acme.test", "unknown_timeout", "initial timeout", now),
)

later = (datetime.datetime.now(datetime.UTC) + datetime.timedelta(seconds=1)).strftime(
    "%Y-%m-%d %H:%M:%S"
)
